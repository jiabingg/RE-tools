...
"""
Small UI app to query monthly cumulative production/injection by completion
for a list of WELL API numbers.
...
This file reuses the same OracleConnectionManager pattern and connection
values as in your existing app. You can drop this alongside your project
and run it as a standalone utility.
"""
...
# UI App
# ---------------------------
class CumMonthlyApp(tb.Window):
    def __init__(self):
        super().__init__(themename="flatly")
        self.title("Monthly Cumulative by Completion")
    ...
    def run_query(self):
        ...
        sql = f"""
            SELECT
                cd.cmpl_nme,
                cmcf.eftv_dttm,
                cmcf.aloc_cum_oil_prod_vol_qty,
                cmcf.aloc_cum_gros_prod_vol_qty,
                cmcf.aloc_cum_wtr_inj_vol_qty,
                cmcf.aloc_cum_stm_inj_vol_qty
            FROM cmpl_dmn cd
            JOIN dwrptg.cmpl_mnly_cum_fact cmcf
                ON cd.cmpl_fac_id = cmcf.cmpl_fac_id
            WHERE cd.actv_indc = 'Y'
              AND cmcf.eftv_dttm = TRUNC(SYSDATE, 'MONTH')
              AND cd.well_api_nbr IN ({in_list})
        """
        ...
        try:
            ...
            # Normalize date column and column order/casing
