    ...
    def display_results(self, df, apply_global_sort=True):
        ...
        if apply_global_sort:
            ...
            if len(available_sort_columns) > 0:
                ...
                # Suppress warning if columns aren't found, just skip sort
                pass 
...
class EngineeringStringPage(Page):
    ...
    def go_to_summary_page(self):
        self.controller.shared_data["selected_fields"] = [] 
    ...
    def execute_field_query(self):
        ...
        join well_dmn wd on cd.well_fac_id = wd.well_fac_id 
...
class PerformanceSummaryPage(Page):
    ...
    def create_widgets(self):
        ...
        query_calc_frame.columnconfigure(0, weight=1) 
        query_calc_frame.columnconfigure(1, weight=1) 
        ...
        add_calc_row("Total INJ wells (Active/TA):", "total_inj") 
...
class ProductionInjectionPage(Page):
    ...
    def create_widgets(self):
        ...
        # New "Next" button to navigate to the new InjectionPressurePage (Page 7)
        next_button = tb.Button(nav_frame, text="Next (Inj Pressure Data)", command=lambda: self.controller.show_page(InjectionPressurePage), bootstyle="primary")
        next_button.grid(row=0, column=1, padx=10)
...
# NEW Page 7: Injection Data Page
class InjectionPressurePage(Page):
    def __init__(self, parent, controller):
        super().__init__(parent, controller)
        self.create_widgets()
        self.conn_manager = OracleConnectionManager()
    ...
    def create_widgets(self):
        ...
        pull_data_btn = tb.Button(self, text="Pull Data", command=self.pull_data, bootstyle="info")
        pull_data_btn.pack(pady=10)
    ...
    def pull_data(self):
        ...
        sql_query = f"""
        select wd.well_nme, wd.well_api_nbr,cd.cmpl_nme, cd.cmpl_fac_id, avg(case when cf.aloc_stm_inj_vol_qty > 0 then
        cf.aloc_stm_inj_vol_qty end) as avg_stm_inj_vol,
...
class MainApplication(tb.Window):
    def __init__(self):
        ...
        self.frames = {}
        # Added InjectionPressurePage to the tuple
        for F in (WellAPIInputPage, WellBasicDataPage, EngineeringStringPage, PerformanceSummaryPage, TubingPressurePage, ProductionInjectionPage, InjectionPressurePage):
