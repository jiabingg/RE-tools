FRACTURE DATA PDF SCANNER - VERSION 1.2
=======================================
...
SCAN SCOPE
----------
The GUI now includes a "Scan scope" dropdown with two choices:

All fracture-related data
    Uses the full screening rules for step-rate tests, mini-frac/DFIT, leak-off
    tests, hydraulic-fracture treatments, fracture gradient, ISIP, FCP/closure
    pressure, breakdown pressure, treating pressure, rates, and proppant.

Step-rate test / ISIP / FCP only
    Focuses the classification and match details on only:
    - step-rate tests;
    - ISIP (instantaneous shut-in pressure); and
    - FCP (fracture closure pressure).

    In this focused mode, mini-frac, DFIT, leak-off, hydraulic-fracture,
    breakdown-pressure, treating-pressure, and proppant-only matches are
    ignored. Fracture-gradient and rate values may still be shown when they
    support a detected step-rate test, but they do not qualify a PDF by
    themselves.

    Keep early stopping checked for the fastest screening of hundreds of PDFs.
    Uncheck it when you need the scanner to collect every matching step-rate,
    ISIP, and FCP page in each PDF.

The results table and Excel output include an "FCP / Closure Pressure" field.
The scanner recognizes both the full phrase and the acronym FCP.

RECOMMENDED SCAN SETTINGS
-------------------------
- Scan scope: Step-rate test / ISIP / FCP only when those are the only records
  needed; otherwise use All fracture-related data
...
YES - Test/treatment data
    ...
    In focused mode, this means an actual step-rate record or a measured
    numeric ISIP/FCP value was found.
