...
The GUI includes a focused scan scope that searches only for:
...
SCAN_SCOPE_ALL = "all"
SCAN_SCOPE_TARGET_ONLY = "step_rate_isip_fcp"
SCAN_SCOPE_ALL_LABEL = "All fracture-related data"
SCAN_SCOPE_TARGET_LABEL = "Step-rate test / ISIP / FCP only"
SCAN_SCOPE_LABEL_TO_VALUE = {
    SCAN_SCOPE_ALL_LABEL: SCAN_SCOPE_ALL,
    SCAN_SCOPE_TARGET_LABEL: SCAN_SCOPE_TARGET_ONLY,
}
SCAN_SCOPE_VALUE_TO_LABEL = {value: label for label, value in SCAN_SCOPE_LABEL_TO_VALUE.items()}
TARGET_CATEGORIES = {"Step-rate test", "ISIP", "Closure pressure"}
...
class ScanResult:
    ...
    def summary_row(self) -> dict[str, object]:
        return {
            ...
            "FCP / Closure Pressure": self.closure_pressures,
...
KEYWORD_RULES: tuple[KeywordRule, ...] = (
    KeywordRule(
        ...
        "step-rate test",
        _rx(
            r"\b(?:step[\s\-_/]*rate|rate[\s\-_/]*step)"
            r"(?:\s+(?:injection\s+)?tests?)?\b"
        ),
        9,
        True,
    ),
    ...
    KeywordRule(
        "Closure pressure",
        "FCP or fracture closure pressure",
        _rx(
            r"\b(?:f\s*\.?\s*c\s*\.?\s*p\s*\.?|"
            r"fracture\s+closure\s+(?:pressure|stress)|closure\s+pressure)\b"
        ),
        6,
        True,
    ),
    KeywordRule(
        ...
        "ISIP",
        _rx(
            r"\b(?:i\s*\.?\s*s\s*\.?\s*i\s*\.?\s*p\s*\.?|"
            r"i\s*\.?\s*s\s*\.?\s*p\s*\.?|"
            r"instantaneous\s+shut[\s\-]*in\s+pressure)\b"
        ),
        6,
        True,
    ),
...
ISIP_TERM_PATTERN = (
    r"(?:i\s*\.?\s*s\s*\.?\s*i\s*\.?\s*p\s*\.?|"
    r"i\s*\.?\s*s\s*\.?\s*p\s*\.?|"
    r"instantaneous\s+shut[\s\-]*in\s+pressure)"
)
FCP_TERM_PATTERN = (
    r"(?:f\s*\.?\s*c\s*\.?\s*p\s*\.?|"
    r"fracture\s+closure\s+(?:pressure|stress)|closure\s+pressure)"
)

ISIP_PATTERNS = (
    _rx(
        rf"\b{ISIP_TERM_PATTERN}\b[^\d]{{0,30}}" + PRESSURE_NUMBER
    ),
    _rx(
        PRESSURE_NUMBER
        + rf"[^a-z0-9]{{0,20}}\b{ISIP_TERM_PATTERN}\b"
    ),
)

CLOSURE_PATTERNS = (
    _rx(rf"\b{FCP_TERM_PATTERN}\b[^\d]{{0,40}}" + PRESSURE_NUMBER),
    _rx(PRESSURE_NUMBER + rf"[^a-z0-9]{{0,25}}\b{FCP_TERM_PATTERN}\b"),
)
...
ACTUAL_MARKER_RE = _rx(
    r"\b(?:(?:was|were)\s+conducted|conducted\s+(?:on|at)|"
    r"(?:was|were)\s+performed|performed\s+(?:on|at)|test\s+was\s+performed|"
    r"pumped|pump\s+and\s+flush|"
...
def is_target_only_scope(scan_scope: str) -> bool:
    return scan_scope == SCAN_SCOPE_TARGET_ONLY
...
def analyze_page(
    ...
    target_only = is_target_only_scope(scan_scope)
    active_rules = (
        rule for rule in KEYWORD_RULES
        if not target_only or rule.category in TARGET_CATEGORIES
    )
    ...
    if evidence.isip_values:
        evidence.categories.add("ISIP")
        evidence.category_weights["ISIP"] = max(
            evidence.category_weights.get("ISIP", 0), 6
        )
    if evidence.closure_values:
        evidence.categories.add("Closure pressure")
        evidence.category_weights["Closure pressure"] = max(
            evidence.category_weights.get("Closure pressure", 0), 6
        )

    if target_only:
        evidence.breakdown_values = []
        evidence.treating_values = []
        evidence.proppant_values = []
    else:
    ...
    explicit_actual = bool(ACTUAL_MARKER_RE.search(flat))
    if target_only:
        target_numeric = bool(evidence.isip_values or evidence.closure_values)
        evidence.actual_marker = explicit_actual or (
            target_numeric
            and not evidence.planned_marker
            and not evidence.assumed_marker
        )
    else:
        evidence.actual_marker = explicit_actual or bool(
    ...
    synthetic_values = (
        ...
        + [("FCP / Closure pressure", value) for value in evidence.closure_values]
    )
    if target_only:
        if "Step-rate test" in evidence.categories:
            synthetic_values += (
                [("Step-rate supporting gradient", value) for value in evidence.gradients]
                + [("Step-rate supporting rate", value) for value in evidence.rate_values]
            )
    else:
        synthetic_values += (
...
def preliminary_strong_evidence(
    ...
    if is_target_only_scope(scan_scope):
        target_actual = any(
            page.actual_marker and bool(page.categories & TARGET_CATEGORIES)
            for page in pages
        )
        step_rate = "Step-rate test" in categories
        measured_target_pressure = any(
            page.actual_marker and bool(page.isip_values or page.closure_values)
            for page in pages
        )
        if measured_target_pressure:
            return True
        if step_rate and (target_actual or gradients or rates):
            return True
        return False
...
def classify_document(
    ...
    if is_target_only_scope(scan_scope):
        step_rate = "Step-rate test" in categories
        target_found = bool(categories & TARGET_CATEGORIES)
        target_actual = any(
            p.actual_marker and bool(p.categories & TARGET_CATEGORIES)
            for p in pages
        )
        target_planned = any(
            p.planned_marker and bool(p.categories & TARGET_CATEGORIES)
            for p in pages
        )
        target_assumed = any(
            p.assumed_marker and bool(p.categories & TARGET_CATEGORIES)
            for p in pages
        )
        score = sum(category_weights.values())
        score += 4 if isip else 0
        score += 4 if closure else 0
        score += 5 if step_rate and gradients else 0
        score += 2 if step_rate and rates else 0
        score += 3 if target_actual else 0

        if (isip or closure) and target_planned and not target_actual:
            status = "POSSIBLE - Planned only"
            note = "Focused scan found a planned/anticipated numeric ISIP or FCP value, not a clear measured result."
        elif (isip or closure) and target_assumed and not target_actual:
            status = "REFERENCE/ASSUMED ONLY"
            note = "Focused scan found an assumed, estimated, or reference ISIP/FCP value rather than a clear measured result."
        elif isip or closure:
            ...
            if closure:
                found.append("FCP")
            note = "Focused scan found numeric " + " and ".join(found) + " data."
        elif step_rate and (target_actual or gradients or rates):
            status = "YES - Test/treatment data"
            note = "Focused scan found an actual step-rate test record or supporting rate/gradient data."
        elif target_found and target_planned and not target_actual:
            status = "POSSIBLE - Planned only"
            note = "Focused scan found a planned/requested step-rate, ISIP, or FCP reference without clear measured data."
        elif target_found:
            status = "POSSIBLE - Review"
            note = "Focused scan found a step-rate, ISIP, or FCP keyword, but no clear numeric target value was extracted."
        else:
            status = "NO CLEAR DATA"
            note = "No step-rate test, ISIP, or FCP evidence was found in the focused scan."
...
def scan_pdf(
    ...
    try:
        ...
        for page_index in range(total_pages):
            ...
            pages_scanned += 1
            evidence = analyze_page(text, page_number, source, options.scan_scope)
            ...
            if (
                ...
                or evidence.gradients
                or evidence.isip_values
                or evidence.closure_values
            ):
                metadata_text_parts.append(text[:8000])

            if options.stop_after_strong_evidence and preliminary_strong_evidence(
                page_evidence, options.scan_scope
            ):
                break
    except Exception as exc:
        details = [
    ...
    status, score, notes = classify_document(
        page_evidence, image_only_not_ocrd, options.scan_scope
    )
    ...
    target_only = is_target_only_scope(options.scan_scope)
    has_step_rate = "Step-rate test" in categories
    if target_only and not has_step_rate:
        # Gradient and rate values are retained only as supporting evidence for
        # a step-rate test. They do not independently qualify a focused scan.
        gradients = []
        rates = []
        ...
        matched_pages = sorted(
            {
                ...
                or p.isip_values
                or p.closure_values
                or (has_step_rate and (p.gradients or p.rate_values))
            }
        )
    else:
        matched_pages = sorted(
        ...
        rows.append(
            {
                ...
                "FCP / Closure Pressure": "; ".join(unique_preserve(v for i in items for v in i.closure_pressures.split(";") if v.strip())),
...
def export_xlsx(results: Sequence[ScanResult], output_path: Path, settings: Optional[dict[str, object]] = None) -> None:
    ...
    well_headers = list(well_rows[0].keys()) if well_rows else [
        ...
        "ISIP", "FCP / Closure Pressure", "Breakdown Pressure", "Treating Pressure",
...
class FractureScannerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.scan_scope_var = tk.StringVar(value=SCAN_SCOPE_ALL_LABEL)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(source_frame, text="Scan scope:").grid(row=2, column=0, sticky="w", pady=4)
        ttk.Combobox(
            source_frame,
            textvariable=self.scan_scope_var,
            state="readonly",
            values=(SCAN_SCOPE_ALL_LABEL, SCAN_SCOPE_TARGET_LABEL),
            width=38,
        ).grid(row=2, column=1, sticky="w", pady=4)
        ttk.Label(
            source_frame,
            text="Focused mode ignores mini-frac, hydraulic-fracture, breakdown, treating-pressure, and proppant-only matches.",
            wraplength=430,
        ).grid(row=2, column=2, sticky="w", padx=(8, 0), pady=4)

        ttk.Label(source_frame, text="OCR DPI:").grid(row=3, column=0, sticky="w", pady=4)
        dpi_combo = ttk.Combobox(source_frame, textvariable=self.dpi_var, width=8, state="readonly", values=(120, 150, 180, 200, 250, 300))
        dpi_combo.grid(row=3, column=1, sticky="w", pady=4)

        worker_frame = ttk.Frame(source_frame)
        worker_frame.grid(row=3, column=2, sticky="w", padx=(8, 0), pady=4)
        ...
        ttk.Label(source_frame, text="Tesseract executable:").grid(row=4, column=0, sticky="w", padx=(0, 8), pady=4)
        ttk.Entry(source_frame, textvariable=self.tesseract_var).grid(row=4, column=1, sticky="ew", pady=4)
        ttk.Button(source_frame, text="Browse...", command=self._browse_tesseract).grid(row=4, column=2, padx=(8, 0), pady=4)
        ...
        columns = (
            "status", "score", "api", "well", "categories", "gradient", "isip", "fcp", "pages", "ocr", "file"
        )
    ...
    def _validate_options(self) -> Optional[tuple[Path, ScanOptions, int]]:
        ...
        scan_scope = SCAN_SCOPE_LABEL_TO_VALUE.get(self.scan_scope_var.get())
        if scan_scope is None:
            messagebox.showerror(APP_TITLE, "Please select a valid scan scope.")
            return None
        if self.ocr_var.get():
            if pytesseract is None:
    ...
    def _start_scan(self) -> None:
        ...
        scope_label = SCAN_SCOPE_VALUE_TO_LABEL.get(options.scan_scope, options.scan_scope)
        self.status_var.set(
            f"Preparing to scan {len(pdfs)} PDF file(s) using: {scope_label}..."
        )
    ...
    def _show_selected_details(self, _event: object = None) -> None:
        ...
        lines = [
            ...
            f"FCP / CLOSURE PRESSURE: {result.closure_pressures or '(none)'}",
    ...
    def _export_results(self) -> None:
        ...
        selected = filedialog.asksaveasfilename(
            ...
            initialfile=(
                "step_rate_isip_fcp_scan_results.xlsx"
                if self.scan_scope_var.get() == SCAN_SCOPE_TARGET_LABEL
                else "fracture_data_scan_results.xlsx"
            ),
        ...
        settings = {
            ...
            "Scan Scope": self.scan_scope_var.get(),
...
def run_cli(args: argparse.Namespace) -> int:
    ...
    options = ScanOptions(
        ...
        scan_scope=(SCAN_SCOPE_TARGET_ONLY if args.target_only else SCAN_SCOPE_ALL),
    )
    if options.use_ocr:
        try:
            configure_tesseract(options.tesseract_path)
    ...
    output = Path(args.output) if args.output else folder / default_output_name
    settings = {
        ...
        "Scan Scope": SCAN_SCOPE_VALUE_TO_LABEL[options.scan_scope],
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--target-only",
        action="store_true",
        help="Scan only for step-rate tests, ISIP, and FCP/closure pressure.",
    )
    return parser
