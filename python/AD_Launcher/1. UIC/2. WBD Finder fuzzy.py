...
try:
    ...
    print("Warning: The 'oracledb' module is not installed. Database query features will be unavailable.")

try:
    import pandas as pd
except ImportError:
    print("Warning: The 'pandas' module is not installed. Step 2 analysis and export features will be unavailable.")
...
class OracleConnectionManager:
    ...
    def get_connection(self, name):
        ...
        if oracledb is None:
            raise RuntimeError("Oracle DB support is unavailable because the 'oracledb' module is not installed.")
...
class FinderPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(main_frame, text="12-Digit API + Wellbore Codes:", font=("Helvetica", 11, "bold"), bg="#eaf0f2").grid(row=3, column=0, sticky="nw", pady=(15, 5))
    ...
    def start_processing_thread(self):
        if self.source_listbox.size() == 0 or not self.target_path.get() or not self.file_list_textbox.get('1.0', tk.END).strip():
            messagebox.showerror("Error", "Please provide at least one source folder, a target folder, and at least one 12-digit API + wellbore code.")
            return
    ...
    def process_files(self, result_queue):
        ...
        try:
            ...
            def filename_matches_task(filename, api, bore):
                """Return True when a PDF filename contains the requested API and wellbore code.

                Matching rules:
                - The 10-digit API may appear anywhere in the filename.
                - For wellbore code 00, the API alone is sufficient because many main-bore
                  files omit _00 from the filename.
                - For a nonzero wellbore code, accept common separators such as underscore,
                  hyphen, or spaces after the API, for example:
                    0402938307_01.pdf
                    BE 52 - 0402938307-01.pdf
                    BE 52 - 0402938307 01.pdf
                """
                ...
                if api not in normalized_name:
                    return False

                if bore == "00":
                    return True

                # Check the text following each API occurrence so unrelated numbers
                # elsewhere in the well name do not create a false match.
                start = 0
                while True:
                    api_index = normalized_name.find(api, start)
                    if api_index == -1:
                        return False

                    suffix = normalized_name[api_index + len(api):]
                    suffix = suffix.lstrip(" _-.")
                    if suffix.startswith(bore):
                        # Require the bore code to end at a separator or end of filename.
                        after_bore = suffix[len(bore):]
                        if not after_bore or not after_bore[0].isdigit():
                            return True
                ...
                for dirpath, _, filenames in os.walk(source_dir):
                    for filename in filenames:
                        for task_key, data in tasks.items():
                            if filename_matches_task(filename, data['api'], data['bore']):
...
class AbandonmentCheckPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.results_df = pd.DataFrame() if pd is not None else []

        if pd is None:
            self.run_query_button.config(state=tk.DISABLED)
            self.export_button.config(state=tk.DISABLED)
            tk.Label(main_frame, text="Warning: pandas is not installed. Step 2 features are unavailable.", fg="red", bg="#eaf0f2").grid(row=3, column=0, columnspan=2, sticky="w", pady=(5, 10))
    ...
    def load_data_and_prepare(self):
        ...
        try:
            ...
            if pd is None:
                self.result_tree["columns"] = ["UWI", "FileName", "FileModifiedDate", "WellStatus"]
                for item in self.copied_files_data:
                    self.result_tree.insert("", "end", values=[
                        item.get('UWI', ''),
                        item.get('FileName', ''),
                        item.get('FileModifiedDate', ''),
                        'Unavailable'
                    ])
                return
    ...
    def run_abandonment_query(self):
        ...
        if pd is None:
            messagebox.showerror("Missing Dependency", "The pandas module is required for Step 2 and is not installed.")
            return

        if oracledb is None:
            messagebox.showerror("Missing Dependency", "The oracledb module is required for database queries and is not installed.")
            return
