"""
Recent Drilled Wells Performance Viewer  (v9)
===============================================
Tabs:
...
Fixes in v9:
  - Clears stale dashboard/table/chart data before each new pull.
  - Shows an explicit empty dashboard when no wells are found.
  - Clears old data on query failure.
  - Uses statistics.median for proper median calculation.
...
# All SQL queries use :spud_date and :spud_end for the drilled-well date range.
SQL_WELL_INVENTORY = """
WITH base AS (
    ...
    WHERE wd.bore_start_dttm >= :spud_date
      ...
      AND cd.actv_indc = 'Y'
      AND NVL(cd.cmpl_state_type_desc, 'X') NOT IN ('Permanently Abandoned', 'Proposed', 'Canceled')
)
...
def mk_filter(parent, fv, ev, cb):
    ...
    fc.pack(side="left", padx=(4, 12))
    fc.bind("<<ComboboxSelected>>", cb)

    ttk.Label(bar, text="Engr Strategy:").pack(side="left")
    ec = ttk.Combobox(bar, textvariable=ev, width=22, state="readonly")
    ec.pack(side="left", padx=(4, 0))
    ec.bind("<<ComboboxSelected>>", cb)
...
class App:
    ...
    # ── Top bar ──────────────────────────────────────────────────────────────
    def _top(self):
        b = ttk.Frame(self.root, padding=(12, 8))
        b.pack(fill="x")

        ttk.Label(b, text="Recent Drilled Wells Performance", style="Header.TLabel").pack(side="left")
        ...
        vc = ttk.Combobox(row1, textvariable=self.vintage_var, width=14, values=vintage_vals, state="readonly")
        ...
        ttk.Label(row1, text="  ").pack(side="left")
        self.pbtn = ttk.Button(row1, text="  Pull Data  ", style="Accent.TButton", command=self._on_pull)
    ...
    def _on_vintage(self, _=None):
        ...
        if v == "Last 12 Months":
            ...
            self.fr_yr.set(str(ago.year))
            self.fr_mo.set(f"{ago.month:02d}")
            self.fr_dy.set(f"{ago.day:02d}")

            now = datetime.now()
            self.to_yr.set(str(now.year))
            self.to_mo.set(f"{now.month:02d}")
            self.to_dy.set(f"{now.day:02d}")
        else:
            yr = int(v)
            self.fr_yr.set(str(yr))
            self.fr_mo.set("01")
            self.fr_dy.set("01")
            self.to_yr.set(str(yr))
            self.to_mo.set("12")
            self.to_dy.set("31")
    ...
    # ── Tab 0: Dashboard ─────────────────────────────────────────────────────
    def _t0_dash(self):
        self.dash_frame = ttk.Frame(self.f0)
        self.dash_frame.pack(fill="both", expand=True, padx=8, pady=8)
    ...
    def _t1_inv(self):
        self.i_fv = tk.StringVar(value="All")
        self.i_ev = tk.StringVar(value="All")
        self.i_fc, self.i_ec = mk_filter(self.f1, self.i_fv, self.i_ev, self._fi)

        top = ttk.Frame(self.f1, padding=(8, 2))
        top.pack(fill="x")

        self.i_lb = ttk.Label(top, text="No data.", style="Sub.TLabel")
        self.i_lb.pack(side="left")
    ...
    def _t2_wt(self):
        self.w_fv = tk.StringVar(value="All")
        self.w_ev = tk.StringVar(value="All")
        self.w_fc, self.w_ec = mk_filter(self.f2, self.w_fv, self.w_ev, self._fw)

        top = ttk.Frame(self.f2, padding=(8, 2))
        top.pack(fill="x")

        self.w_lb = ttk.Label(top, text="No data.", style="Sub.TLabel")
        self.w_lb.pack(side="left")
    ...
    def _t3_ch(self):
        o = ttk.Frame(self.f3)
        o.pack(fill="both", expand=True, padx=8, pady=8)
        ...
        fs = tk.Frame(left, bg=self.PNL)
        fs.pack(fill="x", padx=10, pady=(8, 0))
        ...
        self.c_fc.pack(fill="x", pady=(2, 0))
        self.c_fc.bind("<<ComboboxSelected>>", self._cf)

        es = tk.Frame(left, bg=self.PNL)
        es.pack(fill="x", padx=10, pady=(4, 0))
        ...
        self.c_ec.pack(fill="x", pady=(2, 0))
        self.c_ec.bind("<<ComboboxSelected>>", self._ce)

        tk.Frame(left, bg=self.BDR, height=1).pack(fill="x", padx=10, pady=6)

        wh = tk.Frame(left, bg=self.PNL)
        wh.pack(fill="x", padx=10)
        tk.Label(wh, text="SELECT WELL", font=("Segoe UI", 8, "bold"), fg=self.ACC, bg=self.PNL).pack(side="left")
        self.wc_lb = tk.Label(wh, text="", font=("Segoe UI", 8), fg="#888", bg=self.PNL)
        self.wc_lb.pack(side="right")

        tf = tk.Frame(left, bg=self.PNL)
        ...
        self.wtr.column("WELL", width=150)
        self.wtr.column("TYPE", width=72)
        self.wtr.column("PEAK_OIL", width=62, anchor="e")

        sb = ttk.Scrollbar(tf, orient="vertical", command=self.wtr.yview)
        self.wtr.configure(yscrollcommand=sb.set)
        self.wtr.pack(side="left", fill="both", expand=True)
        sb.pack(side="right", fill="y")
        self.wtr.tag_configure("even", background="#f0f4f8")
        self.wtr.tag_configure("odd", background="white")
        ...
        ct = tk.Frame(right, bg=self.PNL)
        ct.pack(fill="x", padx=10, pady=(8, 0))
        self.ch_lb = tk.Label(ct, text="Select a well", font=("Segoe UI", 9), fg="#888", bg=self.PNL)
        self.ch_lb.pack(side="left")

        ca = tk.Frame(right, bg=self.PNL)
        ...
        if HAS_MPL:
            ...
            tb = tk.Frame(ca, bg=self.PNL)
            tb.pack(fill="x")
            NavigationToolbar2Tk(self.canvas, tb).update()

    def _mt(self, p):
        t = ttk.Treeview(p, selectmode="extended")
        vs = ttk.Scrollbar(p, orient="vertical", command=t.yview)
        hs = ttk.Scrollbar(p, orient="horizontal", command=t.xview)
        ...
        t.tag_configure("even", background="#f0f4f8")
        t.tag_configure("odd", background="white")
        return t
    ...
    def _set(self, m):
        self.sb.config(text=m)
        self.root.update_idletasks()

    def _err(self, m):
        self._set("Query failed.")
        messagebox.showerror("Error", m)

    def _empty_dashboard(self, message="No wells found for selected date range"):
        if not HAS_MPL or not hasattr(self, "dash_fig"):
            return

        self.dash_fig.clear()
        ax = self.dash_fig.add_subplot(111)
        ax.axis("off")
        ax.text(
            0.5,
            0.55,
        ...
        if self.spud and self.spud_end:
            end_disp = self.spud_end - relativedelta(days=1)
            ax.text(
                0.5,
                0.47,
                f"Date range: {self.spud:%Y-%m-%d} to {end_disp:%Y-%m-%d}",
                ha="center",
                va="center",
                fontsize=10,
                color="#888",
    ...
    def _clear_all_views(self, status_msg=None):
        """Clear visible UI so stale results are not left on screen while a new query runs."""
        for tree in [getattr(self, "t1", None), getattr(self, "t2", None)]:
            if tree:
                tree.delete(*tree.get_children())

        if hasattr(self, "i_lb"):
            self.i_lb.config(text="No data.")
        if hasattr(self, "w_lb"):
            self.w_lb.config(text="No data.")
        if hasattr(self, "wc_lb"):
            self.wc_lb.config(text="0 well(s)")
        if hasattr(self, "ch_lb"):
            self.ch_lb.config(text="Select a well", fg="#888")

        if hasattr(self, "wtr"):
            self.wtr.delete(*self.wtr.get_children())

        for cb, var in [
            (getattr(self, "i_fc", None), getattr(self, "i_fv", None)),
            (getattr(self, "i_ec", None), getattr(self, "i_ev", None)),
            (getattr(self, "w_fc", None), getattr(self, "w_fv", None)),
            (getattr(self, "w_ec", None), getattr(self, "w_ev", None)),
            (getattr(self, "c_fc", None), getattr(self, "c_fv", None)),
            (getattr(self, "c_ec", None), getattr(self, "c_ev", None)),
        ]:
            if cb:
                cb["values"] = ["All"]
            if var:
        ...
        self._empty_dashboard("Loading selected date range ...")

        if HAS_MPL and hasattr(self, "fig"):
            self.fig.clear()
            self.canvas.draw()

        if status_msg:
            self._set(status_msg)
    ...
    def _on_pull(self):
        try:
            ...
            to_date = datetime(int(self.to_yr.get()), int(self.to_mo.get()), int(self.to_dy.get()))
            self.spud_end = to_date + relativedelta(days=1)  # inclusive To date
        except Exception:
            messagebox.showerror("Bad Date", "Invalid date range.")
            return

        if self.spud_end <= self.spud:
            messagebox.showerror("Bad Range", "'To' date must be after 'From' date.")
            return

        self.pbtn.config(state="disabled")
        end_disp = self.spud_end - relativedelta(days=1)

        # Critical bug fix: clear old results before the new background query starts.
        self._clear_all_views(f"Querying wells spudded {self.spud:%Y-%m-%d} to {end_disp:%Y-%m-%d} ...")
    ...
    def _bg(self):
        try:
            ...
            ic, ir = qry(SQL_WELL_INVENTORY, p)
            self.raw_ic = ic
            self.raw_ir = ir

            self.root.after(0, self._set, "Querying well tests ...")
            lc, lr = qry(SQL_WELL_TESTS_LATEST, p)
            pc, pr = qry(SQL_WELL_TESTS_PEAK, p)
            mc, mr = self._mtest(lc, lr, pc, pr)
            self.raw_wc = mc
            self.raw_wr = mr

            self.root.after(0, self._set, "Finding wells with data ...")
            _, prod = qry(SQL_PRODUCERS_WITH_TESTS, p)
            _, inj = qry(SQL_INJECTORS_WITH_DATA, p)
            ...
            # Important: schedule one safe UI-finish function.
            # If the UI refresh crashes, it will show the actual error instead of staying on Loading.
    ...
    def _finish_load(self):
        """Finish a background pull on the Tkinter thread, safely."""
        try:
            self._pop_all()
        except Exception as e:
            self._handle_query_error(f"UI refresh failed after the query completed:\n\n{e}")
        finally:
            self.pbtn.config(state="normal")

    def _handle_query_error(self, msg):
        """Show query/UI errors and clear stale loading state."""
        self.raw_ic = []
        self.raw_ir = []
        self.raw_wc = []
        self.raw_wr = []
        self.cw = []
        self._clear_all_views("Query failed. Previous results cleared.")
        self._empty_dashboard("Query or UI refresh failed")
        self.pbtn.config(state="normal")
        messagebox.showerror("Error", msg)

    def _mtest(self, lc, lr, pc, pr):
        pm = {}
        ni = pc.index("WELL_NME")
        ndi = pc.index("PEAK_TEST_DATE")
    ...
    def _pop_all(self):
        ...
        end_disp = self.spud_end - relativedelta(days=1)
        self._set(f"Loaded {len(self.raw_ir)} well(s) for {self.spud:%Y-%m-%d} to {end_disp:%Y-%m-%d}.")
    ...
    def _fi(self, _=None):
        ...
        self.i_ec["values"] = ["All"] + es
        if self.i_ev.get() not in ["All"] + es:
            self.i_ev.set("All")
        self._ai()
    ...
    def _ai(self):
        ...
        fr = [
            r[:11]
            for r in self.raw_ir
            if (fld == "All" or (r[INV_FLD] or "") == fld)
            and (eng == "All" or (r[INV_ENGR] or "") == eng)
        ]
        w = {
            "CMPL_NME": 170,
            "WELL_API_NBR": 110,
            "OPNL_FLD": 100,
            "PRIM_PURP_TYPE_CDE": 70,
            "PRIM_MATL_DESC": 80,
            "ENGR_STRG_NME": 170,
            "CMPL_STATE_TYPE_DESC": 120,
            "CMPL_STATE_EFTV_DTTM": 110,
            "BORE_START_DTTM": 100,
            "INIT_PROD_DTE": 100,
            "INIT_INJ_DTE": 100,
        }
    ...
    def _fw(self, _=None):
        ...
        if self.w_ev.get() not in ["All"] + es:
            self.w_ev.set("All")
        self._aw()
    ...
    def _aw(self):
        ...
        fr = [
            ...
            if (fld == "All" or (r[WT_FLD] or "") == fld)
            and (eng == "All" or (r[WT_ENGR] or "") == eng)
        ]
        w = {
            "WELL_NME": 170,
            "WELL_API_NBR": 110,
    ...
    def _cf(self, _=None):
        ...
        if self.c_ev.get() not in ["All"] + es:
            self.c_ev.set("All")
        self._rwt()
    ...
    def _rwt(self):
        ...
        if ch:
            ...
            if HAS_MPL:
                self.fig.clear()
                self.canvas.draw()
            self.ch_lb.config(text="No wells available", fg="#888")
    ...
    def _gsi(self):
        ...
        nm = self.wtr.item(sel[0], "values")[0]
        fld = self.c_fv.get()
        eng = self.c_ev.get()

        for n, f, eg, p, m, fi, pk in self.cw:
            if n == nm and (fld == "All" or f == fld) and (eng == "All" or eg == eng):
                return n, f, p, m, fi, pk
        return None

    def _ws(self, _=None):
        i = self._gsi()
        if not i or not HAS_MPL:
            return

        nm, fl, pu, mt, fi, pk = i
        self.ch_lb.config(text=f"Loading {nm} ...", fg="#888")
    ...
    def _draw_dashboard(self):
        ...
        inv = self.raw_ir
        if not inv:
            self._empty_dashboard("No wells found for selected date range")
            return
        ...
        n_other = sum(1 for r in inv if r[3] not in ("PROD", "INJ", "OBSN"))
        n_total = len(inv)
        ...
        monthly = defaultdict(lambda: {"PROD": 0, "INJ": 0, "OBSN": 0, "OTHER": 0})
        for r in inv:
            if r[8]:
                purp = r[3] if r[3] in ("PROD", "INJ", "OBSN") else "OTHER"
                monthly[r[8].replace(day=1)][purp] += 1
        ms = sorted(monthly.keys())

        by_fld = defaultdict(lambda: {"PROD": 0, "INJ": 0, "OBSN": 0, "OTHER": 0})
        for r in inv:
            purp = r[3] if r[3] in ("PROD", "INJ", "OBSN") else "OTHER"
            by_fld[r[2] or "Unknown"][purp] += 1

        ip_by_fld = defaultdict(list)
        for r in self.raw_wr:
            if r[-1] and r[-1] > 0 and r[WT_FLD]:
                ip_by_fld[r[WT_FLD]].append(r[-1])
        ...
        # KPI cards
        ax = self.dash_fig.add_subplot(gs[0, 0])
        ax.axis("off")
        cards = [
            (f"{n_total}", "Total Wells Drilled"),
            (f"{n_prod}", "Producers"),
            (f"{n_inj}", "Injectors"),
            (f"{n_obsn}", "Observation"),
            (f"{n_other}", "Other"),
            (f"{avg_ip:.0f}", "Avg Peak IP (BOPD)"),
            (f"{med_ip:.0f}", "Median Peak IP"),
            (f"{max_ip:.0f}", "Max Peak IP"),
            (f"{avg_s2p:.0f}", "Avg Spud-to-Prod (days)"),
        ]
        yp = 1.0
        for val, label in cards:
            ax.text(0.02, yp, val, fontsize=16, fontweight="bold", color=self.ACC, transform=ax.transAxes, va="top")
            ax.text(0.40, yp - 0.01, label, fontsize=9, color="#666", transform=ax.transAxes, va="top")
            yp -= 0.125
        ...
        if ms:
            pv2 = [monthly[m]["PROD"] for m in ms]
            iv = [monthly[m]["INJ"] for m in ms]
            ov = [monthly[m]["OBSN"] for m in ms]
            xl = [m.strftime("%b\n%y") for m in ms]
            x = range(len(ms))
            ax2.bar(x, pv2, color="#27ae60", label="PROD", width=0.7)
            ax2.bar(x, iv, bottom=pv2, color="#2980b9", label="INJ", width=0.7)
            b2 = [p + i for p, i in zip(pv2, iv)]
            ax2.bar(x, ov, bottom=b2, color="#e67e22", label="OBSN", width=0.7)
            ax2.set_xticks(list(x))
            ax2.set_xticklabels(xl, fontsize=7)
        ...
        if fn:
            y = range(len(fn))
            pv3 = [by_fld[f]["PROD"] for f in fn]
            iv3 = [by_fld[f]["INJ"] for f in fn]
            ov3 = [by_fld[f]["OBSN"] for f in fn]
            ax3.barh(y, pv3, color="#27ae60", label="PROD", height=0.6)
            ax3.barh(y, iv3, left=pv3, color="#2980b9", label="INJ", height=0.6)
            lf = [p + i for p, i in zip(pv3, iv3)]
            ax3.barh(y, ov3, left=lf, color="#e67e22", label="OBSN", height=0.6)
            ax3.set_yticks(list(y))
            ax3.set_yticklabels(fn, fontsize=8)
            ax3.invert_yaxis()
        ...
        if pv:
            ...
            ax4.set_xlabel("Peak Oil IP (BOPD)", fontsize=9)
            ax4.set_ylabel("Count", fontsize=9)
        else:
            ax4.text(0.5, 0.5, "No peak oil test data", ha="center", va="center", transform=ax4.transAxes, color="#777")
        ...
        if fipn:
            avgs = [sum(ip_by_fld[f]) / len(ip_by_fld[f]) for f in fipn]
            cnts = [len(ip_by_fld[f]) for f in fipn]
            bars = ax5.bar(range(len(fipn)), avgs, color="#27ae60", edgecolor="white", width=0.6)
            ax5.set_xticks(range(len(fipn)))
            ax5.set_xticklabels(fipn, fontsize=7, rotation=30, ha="right")
            for bb, cnt in zip(bars, cnts):
                ax5.text(bb.get_x() + bb.get_width() / 2, bb.get_height() + 1, f"n={cnt}", ha="center", fontsize=7, color="#666")
        else:
            ax5.text(0.5, 0.5, "No peak oil data by field", ha="center", va="center", transform=ax5.transAxes, color="#777")
    ...
    def _dp(self, cols, rows, nm, fl):
        self.fig.clear()
        if not rows:
            self.ch_lb.config(text=f"{nm}: no data.", fg="#c0392b")
            self.canvas.draw()
            return
        ...
        if any(v > 0 for v in oil):
            l, = ax.plot(dt, oil, "o-", color="#27ae60", ms=5, lw=1.5, label="Oil (BOPD)")
            ln.append(l)
        if any(v > 0 for v in wtr):
            l, = ax.plot(dt, wtr, "s-", color="#2980b9", ms=4, lw=1.2, label="Water (BWPD)")
            ln.append(l)

        ax.set_ylabel("Liquid Rate (bbl/d)", fontsize=9)
        ax.tick_params(labelsize=8)
        ax.grid(True, alpha=0.25)

        if any(v > 0 for v in gas):
            a2 = ax.twinx()
            l, = a2.plot(dt, gas, "x--", color="#e74c3c", ms=4, lw=1.2, label="Gas (MCFD)")
            ln.append(l)
            a2.set_ylabel("Gas (MCFD)", fontsize=9, color="#e74c3c")
            a2.tick_params(labelsize=8, labelcolor="#e74c3c")

        if ln:
            ax.legend(ln, [l.get_label() for l in ln], fontsize=8, loc="upper left", framealpha=0.85)

        t = f"{nm}  —  Allocated Well Tests"
        t += f"  ({fl})" if fl else ""
    ...
    def _di(self, cols, rows, nm, fl, mt):
        ...
        if mt == "Steam":
            ...
            ax.fill_between(dt, st, alpha=0.15, color="#e67e22")
            ax.set_ylabel("Steam Inj (bbl/d)", fontsize=9)
        elif mt == "Water":
            ax.plot(dt, wt, color="#2980b9", lw=0.9, alpha=0.85, label="Water (bbl/d)")
            ax.fill_between(dt, wt, alpha=0.15, color="#2980b9")
            ax.set_ylabel("Water Inj (bbl/d)", fontsize=9)
        else:
            if any(v > 0 for v in st):
                ax.plot(dt, st, color="#e67e22", lw=0.9, label="Steam")
            if any(v > 0 for v in wt):
                ax.plot(dt, wt, color="#2980b9", lw=0.9, label="Water")
